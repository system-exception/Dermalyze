...
function getRiskColor(severity: RiskSeverity): [number, number, number] {
  switch (severity) {
    ...
    case 'moderate': return [217, 119, 6]; // amber-600
    default: return [5, 150, 105]; // emerald-600
  }
}
...
export async function generateDermatologyReport(data: ReportData): Promise<void> {
  ...
  if (data.imageDataUrl) {
    try {
      pdf.addImage(data.imageDataUrl, 'JPEG', col1X + 2, imgBoxY + 2, colW - 4, colW * 0.75 - 4, undefined, 'FAST');
    } catch {
  ...
  pdf.setFontSize(7);
  setColor(pdf, COLORS.secondary);
  const disclaimer = 'This report is generated by an AI-assisted diagnostic tool for educational and screening purposes only. Classification results are probabilistic outputs and do not constitute a definitive medical diagnosis. All findings must be correlated with comprehensive clinical examination, patient history, and histopathological analysis when indicated. Consultation with a board-certified dermatologist is recommended.';
